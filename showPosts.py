...
def postExtremeComments(data, keywords, color):
    ...
    with st.expander("Posts Timeline", expanded=True):
        ...
        for annotatedArgs in AAList:
            annotated_text(annotatedArgs, color=color)
            """
            
            """

def show_posts(brand, stream, df, kw):

    data_brand = df[df['brand']==brand]
    data = data_brand[data_brand['stream']==stream]



    keywords_brand = kw[brand]
    keywords = keywords_brand[stream]


    positiveData = data[data["sentiment_prediction"] == 1].sort_values(by=["sentiment_probability","votes"], ascending=[False,False])
    negativeData_raw = data[data["sentiment_prediction"] == -1].sort_values(by=["sentiment_probability","votes"], ascending=[False,False])
    ...
    """
    postExtremeComments(topPos, keywords, posColor)
    """
    
    # Most negative comments:
    """
    postExtremeComments(topNeg, keywords, negColor)

show_posts('WhatsApp','Reddit',df,kwDict)